-- Ryzen Mobile v2 / Delta
-- Создатель: bibizzzana
-- Меню 6 см, перетаскиваемое, сворачиваемое.
-- Разделы: Me (Noclip, Aimbot, Silent Aim), Esp (ESP), Misc (SOON...)
...
-- Основная панель (меню) размер ~6 см (200x280 px)
local mainFrame = Instance.new("Frame")
mainFrame.Size = UDim2.new(0, 200, 0, 280)
mainFrame.Position = UDim2.new(0.5, -100, 0.5, -140) -- центр экрана
...
-- Заголовок с именем создателя и кнопкой свернуть
...
local function makeSection(frame, title, yPos, height)
    ...
    sec.Position = UDim2.new(0.05, 0, yPos, 0)
...
-- Me секция (высота 120)
local meSec = makeSection(mainFrame, "ME", 0.13, 120)
makeToggle(meSec, "Noclip", 0.30, "noclip")
makeToggle(meSec, "Aimbot", 0.55, "aimbot")
makeToggle(meSec, "Silent Aim", 0.80, "silent")

-- Esp секция (высота 70)
local espSec = makeSection(mainFrame, "ESP", 0.57, 70)
makeToggle(espSec, "ESP", 0.30, "esp")

-- Misc секция (высота 60)
local miscSec = makeSection(mainFrame, "MISC", 0.80, 60)
...
-- ===== Функции (без изменений, скопированы из предыдущего) =====
-- ESP
